...
class TestDcElement < Test::Unit::TestCase
  ...
  def test_01_arguments
    ...
    x.postfix = ']'
    assert_equal '[a-b-c]', x.to_s
    x = DcElement.new 'a' , 'b', ['c', 'd'], 'e', join: ':', prefix: '->', postfix: '.'
    assert_equal '->a:b:c:d:e.', x.to_s
    x = DcElement.from %w(a b c), join: '-', fix: '{}'
    assert_equal '{a-b-c}', x.to_s
    x = DcElement.from ['a' , 'b', ['c', 'd'], 'e'], join: ':', prefix: '->', postfix: '.'
    assert_equal '->a:b:c:d:e.', x.to_s
    x = DcElement.from parts: ['a' , 'b', ['c', 'd'], 'e'], join: ':', prefix: '->', postfix: '.'
    assert_equal '->a:b:c:d:e.', x.to_s
    x = DcElement.from 'a' , 'b', ['c', 'd'], 'e', join: ':', prefix: '->', postfix: '.'
    assert_equal '->a:b:c:d:e.', x.to_s
  end
  ...
  def test_03_complex
    ...
    x = DcElement.new 'a', { parts: ['b', 'c'], join: ':', fix: '()'}, nil
    assert_equal 'a(b:c)', x.to_s
    x = DcElement.new 'a', { parts: ['b', 'c'], join: ':', fix: '()'}
    assert_equal '(a:b:c)', x.to_s
  end
